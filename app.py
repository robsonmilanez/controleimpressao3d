...
def build_jobs_list_template_context(
    ...
    *,
    ...
    prepared_jobs = (
        ...
        else prepare_jobs_for_list(fetch_jobs(db, sort_key=sort_key, sort_direction=sort_direction))
    )
    return {
        "jobs": prepared_jobs,
        "materials": materials_list,
        ...
        "sort_urls": build_sort_urls(
            endpoint="jobs",
            column_keys=list(active_filters.keys()),
            current_sort_key=sort_key,
            current_sort_direction=sort_direction,
            extra_params={f"filter_{key}": value for key, value in active_filters.items() if value},
        ),
    }
        ...
        cost_summary["breakdowns"]["energy"] = [
            {
                ...
                "rate": f"{br_decimal(job['print_hours'])} h x R$ {br_money(job['energy_cost_per_hour'])}/h",
        ...
        cost_summary["breakdowns"]["operating"] = [
            {
                "label": printer_label,
                "base": f"{br_decimal(job['print_hours'])} h",
                "rate": f"{br_decimal(job['print_hours'])} h x R$ {br_money(job['operating_cost_per_hour'])}/h",
        ...
        cost_summary["breakdowns"]["dryers"] = [
            {
                ...
                "rate": f"{br_decimal(fallback_dryer_hours)} h x R$ {br_money(job['dryer_cost_per_hour'])}/h",
